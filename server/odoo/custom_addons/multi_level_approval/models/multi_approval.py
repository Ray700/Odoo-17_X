...
class MultiApproval(models.Model):
    ...
    def set_approved(self):
        ...
        #     try:
        #         move_id = self.env[self.res_model].browse(self.res_id)
        #         if move_id.move_type in ['in_invoice','out_invoice']:
        #             move_id.action_post()
    ...
    def action_approve(self):
        ...
        move_id = self.env['account.move'].sudo().browse(recs.res_id)
        for rec in recs:
            if not rec.is_pic:
        ...
        if recs.state == 'Approved' and recs.res_model == 'account.move':
            move_id = self.env['account.move'].sudo().browse(recs.res_id)
            if move_id.move_type in ['in_invoice','out_invoice']:
                move_id.action_post()
        if recs.state == 'Approved' and recs.res_model == 'purchase.order':
            purchase_id = self.env['purchase.order'].sudo().browse(recs.res_id)
            if purchase_id.state == 'draft':
                purchase_id.sudo().write({'state':'department_approve'})
        if recs.state == 'Approved' and recs.res_model == 'purchase.order':
            purchase_id = self.env['purchase.order'].sudo().browse(recs.res_id)
            if purchase_id.state == 'waiting_approve':
                purchase_id.sudo().write({'state':'approved'})
        if ret_act:
            return ret_act
