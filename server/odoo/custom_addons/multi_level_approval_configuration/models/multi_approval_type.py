...
class MultiApprovalType(models.Model):
    ...
    def get_compute_val(self, f_name):
        _logger.info('___ f_name : ', f_name);
        vals = '''
for rec in self:
  rec['{f_name}'] = rec.env['multi.approval.type'].compute_need_approval(rec)
    ...
    def create_view(self, f_name, f_name1, f_name2):
        ...
        if self.view_id:
            weis = self.env['ir.ui.view'].sudo().search([('inherit_id','=',self.get_default_view()),('model','=',self.model_id)])
            approval_views_final = weis.filtered(lambda l:l.name.startswith('approval_view_') and l.id != existed_view.id)
            if approval_views_final:
                approval_views_final.unlink()
            return False
        if existed_view:
            weis = self.env['ir.ui.view'].sudo().search([('inherit_id','=',self.get_default_view()),('model','=',self.model_id)])
            approval_views_final = weis.filtered(lambda l:l.name.startswith('approval_view_') and l.id != existed_view.id)
        ...
        if not view_id:
            raise ValidationError(_('This model has no form view !'))
        ...
        if not wiz_act or not wiz_view_act or not wiz_rework_act:
            raise ValidationError(_('Not found the action !'))
        ...
        weis = self.env['ir.ui.view'].sudo().search([('inherit_id','=',self.get_default_view()),('model','=',self.model_id)])
        approval_views_final = weis.filtered(lambda l:l.name.startswith('approval_view_') and l.id != new_view.id)
        if approval_views_final:
            approval_views_final.unlink()
    ...
    def update_x_field(self, obj, fi, val=True):
        if hasattr(obj, fi):
            ...
            raise ValidationError(_('Something wrong !'))
